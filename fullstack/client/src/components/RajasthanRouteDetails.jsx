...
import './RajasthanRouteDetails.css';

const RajasthanRouteDetails = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`http://localhost:5000/api/rajasthan-routes/${id}`)
            .then(response => {
                if (!response.ok) {
        ...
        <>
            ...
            <section className="details-main">
                ...
                <p className="book-btn-container">
                    <Link to={`/book/rajasthan/${route.id}`} className="book-now-btn">Book Now</Link>
                </p>
            </section>
        </>
    );
...
export default RajasthanRouteDetails;